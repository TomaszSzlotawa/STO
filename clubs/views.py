...
def create_team(request,club_id):
    ...
    for usersClub in usersClubs:
        if usersClub.club == club:
            if usersClub.admin or usersClub.training_coordinator:
                allowed=True
    if allowed:
        if request.method == 'POST':
...
def edit_team(request, team_id):
    ...
    team = get_object_or_404(Team,pk=team_id)
    club = team.club
    in_team_coaching_staff = TeamsCoaching_Staff.objects.filter(team=team, leaving_date = None, coach = request.user)
    allowed = False
    for usersClub in usersClubs:
        if usersClub.club == club:
            if usersClub.admin or usersClub.training_coordinator:
                allowed=True
    if in_team_coaching_staff:
        allowed=True
    if allowed:
...
def club_staff(request, club_id):
    ...
    for usersClub in usersClubs:
        if usersClub.club == club:
            if usersClub.user == request.user:
                allowed=True
    if allowed:
        club_teams = Team.objects.filter(club=club)
...
def delete_player_from_team(request, season_id, player_id):
    ...
    usersClubs, teams = get_data_for_menu(request)
    in_team_coaching_staff = TeamsCoaching_Staff.objects.filter(team=season.team, leaving_date = None, coach = request.user)
    allowed = False
    for usersClub in usersClubs:
        if usersClub.club == player.club:
            if usersClub.admin or usersClub.training_coordinator:
                allowed=True
    if in_team_coaching_staff:
        allowed=True
...
def club_coaching_staff(request, club_id):
    ...
    if allowed:
        ...
        return render(request,'clubs/lack_of_access.html',{'teams':teams,'usersClubs':usersClubs})
...
def return_equipment(request, rent_id):
    ...
    allowed = False
    for usersClub in usersClubs:
        if usersClub.club == rent.equipment.club:
            if usersClub.user == request.user:
                allowed=True
    if allowed:
...
def training_attendance(request, training_id):
    ...
    usersClubs, teams = get_data_for_menu(request)
    training = get_object_or_404(Training, pk=training_id)
    in_team_coaching_staff = TeamsCoaching_Staff.objects.filter(team=training.season.team, leaving_date = None, coach = request.user)
    allowed = False
    for usersClub in usersClubs:
        if usersClub.club == training.season.team.club:
            if usersClub.admin or usersClub.training_coordinator:
                allowed=True
    if in_team_coaching_staff:
        allowed=True
    if allowed:
        if training.start_datatime > datetime.now():
...
def team_attendance_report(request,team_id):
    ...
    if allowed:
        ...
        return render(request,'clubs/team_attendance_report.html',{'now':datetime.now(),'attendance_filter':attendance_filter, 'players_avg':players_avg,'avg_attendance':avg_attendance,'teams':teams,'usersClubs':usersClubs,'players':players,'trainings':trainings, 'team':team,'attendances':attendances, 'season':season})
    else:
        return render(request,'clubs/lack_of_access.html',{'teams':teams,'usersClubs':usersClubs})
...
def player_attendance_report(request, season_id, player_id):
    ...
    if allowed:
        ...
        return render(request,'clubs/player_attendance_report.html',{'now':datetime.now(),'attendance_filter':attendance_filter, 'avg_attendance':avg_attendance,'teams':teams,'usersClubs':usersClubs,'player':player,'trainings':trainings, 'team':team,'attendances':attendances, 'season':season})
    else:
        return render(request,'clubs/lack_of_access.html',{'teams':teams,'usersClubs':usersClubs})
...
def edit_mezocycle(request, mezocycle_id):
    ...
    in_team_coaching_staff = TeamsCoaching_Staff.objects.filter(team=mezocycle.team, leaving_date = None, coach = request.user)
    allowed = False
    for usersClub in usersClubs:
        if usersClub.club == mezocycle.team.club:
            if usersClub.admin or usersClub.training_coordinator:
                allowed=True
    if in_team_coaching_staff:
        allowed=True
